                    ...
                    <table class="table text-md-nowrap" id="example2">
                        <thead>
                            <tr>
                                ...
                                <th class="wd-10p border-bottom-0">Complaint-Date</th>
                        ...
                        <tbody>
                            @foreach ($complaints as $complaint)
                                <tr class="order-{{ $complaint->id }}">
                                    ...
                                    <td>
                                        <button class="btn btn-outline-danger delete_button"
                                            product_id={{ $complaint->id }}
                                            product_name={{ $complaint->name }}>DELETE</button>
                                    </td>
                                    <td>
                                        <button class="btn btn-outline-info back_button" product_id={{ $complaint->id }}
                                            product_name={{ $complaint->name }}>Back</button>
                                    </td>
        ...
        $(document).on('click', '.back_button', function(e) {
            ...
            if (confirm('Back to Requests: ' + product_name)) {
                ...
                $.ajax({
                    type: "POST",
                    method: "GET",
                    url: "{{ route('complaints.show', '') }}" + "/" + product_id,
                    data: {
                        '_token': '{{ csrf_token() }}',
