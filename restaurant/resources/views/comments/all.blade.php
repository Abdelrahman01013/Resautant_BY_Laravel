                    ...
                    <table class="table text-md-nowrap" id="example2">
                        <thead>
                            <tr>
                                ...
                                <th class="wd-15p border-bottom-0">Customer Pactuer</th>
                        ...
                        <tbody>
                            @foreach ($comments as $comment)
                                <tr class="order-{{ $comment->id }}">
                                    <td>{{ $loop->index + 1 }} </td>
                                    <td>{{ $comment->name }} </td>

                                    <td><img src="{{ 'assets_templet/img/customer/' . $comment->photo }} "
                                            alt="{{ $comment->name }}" class="w-25"> </td>

                                    <td>


                                        <button class="btn btn-outline-danger delete_button" product_id={{ $comment->id }}
                                            product_name={{ $comment->name }}>DELETE</button>
                                    </td>
        ...
        $(document).on('click', '.delete_button', function(e) {
            ...
            if (confirm('Delete :  ' + product_name)) {
                ...
                $.ajax({
                    type: "POST",
                    method: "DELETE",
                    url: "{{ route('comments.destroy', '') }}" + "/" + product_id,
                    data: {
                        '_token': '{{ csrf_token() }}',
